...
export async function POST(req: Request) {  
    const body = await req.json() as { prompt: string, negative_prompt: string, options: { size: string, steps: number, cfg_scale: number } };
    ...
    const automatic1111 = createAutomatic1111({
      ...
      const { image } = await generateImage({
        model: automatic1111.image('plantMilkModelSuite_hempII'), // Default Automatic1111 model
        prompt: prompt,
      ...
      return Response.json(process.env.NEXT_PUBLIC_URL+'/api/image/get?location='+location);
}